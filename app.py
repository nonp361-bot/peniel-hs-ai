...
# 📌 [핵심 반영] 과목 세부능력 특기사항 전용 피드백 선택 시 메인 화면 기준표 전면 교체
if selected_feedback_type == "과목세부능력 특기사항 전용 피드백":
    st.info("💡 **과목 세부능력 특기사항 교사 자가점검 7대 핵심 채점기준표 (100점 만점)**")
    
    col_k1, col_c2, col_k3 = st.columns(3)
    with col_k1:
        st.markdown("""
        <div style="background-color: #EFF6FF; border-left: 5px solid #3B82F6; padding: 15px; border-radius: 6px;">
            <h4 style="color: #1E40AF; margin-top:0;">🎯 핵심 교과 역량 (60점)</h4>
            <ul style="margin-bottom:0; padding-left:20px;">
                <li><b>학생의 교과적 역량을 잘 보여주는 기록인가?</b> <span style="color:#DC2626;">(20점)</span></li>
                <li><b>교사의 관찰이 들어간 기록인가?</b> <span style="color:#DC2626;">(20점)</span></li>
                <li><b>교과의 역량이 들어갔는가?</b> <span style="color:#DC2626;">(20점)</span></li>
            </ul>
        </div>
    ...
    with col_c2:
        st.markdown("""
        <div style="background-color: #FEF3C7; border-left: 5px solid #F59E0B; padding: 15px; border-radius: 6px;">
            <h4 style="color: #92400E; margin-top:0;">🚨 신뢰도 및 오염 검증 (20점)</h4>
            <ul style="margin-bottom:0; padding-left:20px;">
                <li><b>학생 간 복붙한 기록이 없는가?</b> <span style="color:#DC2626;">(10점)</span></li>
                <li><b>AI를 너무 돌려 맥락에 맞지 않는 단어나 문장이 들어가지 않았는가?</b> <span style="color:#DC2626;">(10점)</span></li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_k3:
        st.markdown("""
        <div style="background-color: #F3F4F6; border-left: 5px solid #6B7280; padding: 15px; border-radius: 6px;">
            <h4 style="color: #1F2937; margin-top:0;">📝 가독성 및 규정 준수 (20점)</h4>
            <ul style="margin-bottom:0; padding-left:20px;">
                <li><b>가독성이 높은가?</b> <span style="color:#DC2626;">(10점)</span></li>
                <li><b>생기부 기재 금지 사항이 잘 반영되었는가?</b> <span style="color:#DC2626;">(10점)</span></li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

# 📌 그 외 일반 생기부 / 학생용 피드백 탭 선택 시 기존 삼분 배점표 노출
else:
...
if student_file and api_key:
    ...
    if st.button("🔥 선택한 버전으로 AI 정밀 평가 시작하기", type="primary", use_container_width=True):
        with st.spinner(f"🧠 AI 사정관이 [{evaluator_mode}] 관점에서 [{selected_feedback_type}] 맞춤 정밀 검증을 진행 중입니다..."):
            ...
            # -------------------------------------------------------------
            # [분기 1] 과목 세부능력 특기사항 전용 피드백 (수강생 전체/일부 점검용 7대 항목)
            # -------------------------------------------------------------
            if selected_feedback_type == "과목세부능력 특기사항 전용 피드백":
                prompt = f"""
