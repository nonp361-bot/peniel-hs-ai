...
def normalize_feedback_text(text):
    ...
    #    (?<!\d)로 긴 숫자(학번 등) 중간을 번호로 오인하지 않도록 방지
    t = re.sub(r"(?<!\n)(?<!\d)\s*(\d{1,2}[\.\)]\s*\([^)]{1,25}\))", r"\n\1", t)
    # 3) 괄호 태그 없이 "1. '문장'" 형태로 오는 번호 목록도 줄바꿈 삽입
    #    (?<!\d)로 "(20103)"처럼 괄호 안 학번의 뒷자리("03)")를 번호로 오인하지 않도록 방지
    t = re.sub(r"(?<!\n)(?<!\d)\s*(\d{1,2}[\.\)]\s*['\"'])", r"\n\1", t)
    # 4) "(학번) 수정 전:" / "(학번) 수정 후:" / 학번 접두사가 없는 "수정 전:" 모두 처리.
    #    학번(식별자) 접두사가 있다면 함께 앞으로 이동하여, 접두사가 이전 줄에 남지 않도록 함
    t = re.sub(r"\s*((?:\([^)]{1,20}\)\s*)?-?\s*수정\s*(?:전|후)\s*[:：])", r"\n\1", t)
...
def inject_student_ids(improvements_text, revision_text):
    """'보완 및 수정 필요사항'(improvements)에 이미 표기된 "(학번) '문장'" 매핑을 기준으로,
    '수정·보완 추천 문장 예시'(revision_examples)에서 같은 문장이 나오지만 학번 표기가
    누락된 경우 자동으로 찾아서 붙여주는 코드 단 보정 함수.

    AI에게 프롬프트로 여러 차례 요청해도 매번 빠짐없이 지키리라는 보장이 없으므로,
    이미 improvements에서 확보한 "문장 → 학번" 매핑을 근거 삼아 revision_examples에도
    동일하게 반영해 신뢰성을 코드 레벨에서 담보한다."""
    if not improvements_text or not revision_text:
        return revision_text

    quote_to_id = {}
    for m in re.finditer(r"\(([^)\n]{1,20})\)\s*'([^']{2,400})'", improvements_text):
        sid = m.group(1).strip()
        quote = m.group(2).strip()
        quote_to_id.setdefault(quote, sid)
        norm = re.sub(r"\s+", "", quote)
        quote_to_id.setdefault(norm, sid)

    if not quote_to_id:
        return revision_text

    quote_pattern = re.compile(r"'([^']{2,400})'")
    out = []
    idx = 0
    for m in quote_pattern.finditer(revision_text):
        start, end = m.span()
        quote = m.group(1).strip()
        segment_before = revision_text[idx:start]
        tail = segment_before[-40:]
        # 이 인용문 바로 앞(또는 "수정 전/후:" 라벨 바로 앞)에 이미 학번 표기가 있는지 확인
        already_tagged = bool(re.search(r"\([^)\n]{1,20}\)\s*$", tail)) or \
            bool(re.search(r"\([^)\n]{1,20}\)\s*수정\s*(?:전|후)\s*[:：]\s*$", tail))

        sid = quote_to_id.get(quote) or quote_to_id.get(re.sub(r"\s+", "", quote))

        if sid and not already_tagged:
            # "수정 전:"/"수정 후:" 라벨이 바로 앞에 있으면 라벨 앞에 학번을 삽입
            label_match = re.search(r"(수정\s*(?:전|후)\s*[:：]\s*)$", segment_before)
            if label_match:
                cut = label_match.start()
                out.append(segment_before[:cut])
                out.append(f"({sid}) ")
                out.append(segment_before[cut:])
            else:
        ...
        out.append(revision_text[start:end])
        idx = end

    out.append(revision_text[idx:])
    result = "".join(out)

    # "(학번) 수정 전: '...'" 바로 다음에 이어지는 "수정 후:" 줄에 학번 표기가 없다면
    # 같은 학번을 그대로 이어붙여서, 수정 전/후 쌍이 항상 같은 학번으로 짝지어 보이도록 보정
    pair_pattern = re.compile(
        r"(\([^)\n]{1,20}\)\s*수정\s*전\s*[:：]\s*'[^']{2,400}'\s*\n\s*)(수정\s*후\s*[:：])"
    )

    def _add_id_to_next(m):
        before_block = m.group(1)
        after_label = m.group(2)
        id_match = re.match(r"\(([^)\n]{1,20})\)", before_block)
        if id_match:
            return f"{before_block}({id_match.group(1)}) {after_label}"
        return m.group(0)

    result = pair_pattern.sub(_add_id_to_next, result)
    return result
        ...
        if st.button("🔥 선택한 버전으로 AI 정밀 평가 시작하기", type="primary", use_container_width=True, key="start_eval_btn"):
            with st.spinner(f"🧠 AI 사정관이 [{evaluator_mode}] 관점에서 [{selected_feedback_type}] 맞춤 정밀 검증을 진행 중입니다..."):
                ...
                if selected_feedback_type == "과목세부능력 특기사항 전용 피드백":
                    ...
                    - revision_examples 역시 improvements에서 지적한 "모든" 문제 문장에 대해 빠짐없이 수정 전/후 예시를 제공하는 것이 원칙입니다. 다만 완전히 동일한 패턴(동일 원인)으로 그룹핑된 문장들은, 그중 1개 문장에 대해서만 상세한 '수정 전 ➔ 수정 후' 예시를 보여주고, 나머지 문장들은 "동일 유형이므로 같은 방식으로 수정 가능한 문장: (학번) '문장A', (학번) '문장B'" 형태로 간략히 나열하는 방식으로 처리하세요. (문제 진단은 전수로, 수정 예시 시연은 그룹당 대표 1개로)
                    - [매우 중요] revision_examples의 모든 "수정 전"/"수정 후" 문장 앞에도 improvements에서와 동일하게 그 문장이 속한 학번(또는 이름 등 원문의 식별자)을 반드시 표기하세요. 예: "(20103) 수정 전: '...'" / "(20103) 수정 후: '...'" 처럼 수정 전과 수정 후 두 줄 모두에 동일한 학번을 붙여, 어느 학생의 어느 문장을 어떻게 고쳤는지 한눈에 대응되도록 하세요. 학번 표기를 생략하지 마세요.
                    ...
                    [2번 항목(교사의 관찰) 판별 기준 - 세특 전체 관점에서 균형 있게 적용]:
                    - 2번 항목은 "문장 하나하나마다" 교사의 평가·해석 문구가 반드시 붙어있어야 한다는 뜻이 아닙니다. 세특 전체를 놓고 볼 때 학생의 특성과 역량이 구체적으로 드러나는지를 판단하는 항목이며, 반드시 매 문장을 "~보임", "~돋보임" 같은 평가어로 뜯어고칠 필요는 없습니다.
                    - 다음 두 가지를 명확히 구분하세요:
                      (1) 진짜로 밋밋한 행위 나열: 활동의 주제·대상·방식이 전혀 구체적이지 않아, 이 학생이 무엇을 어떻게 했는지, 무엇이 특별한지 전혀 짐작할 수 없는 문장. 예: "~에 대해 조사하여 발표함.", "~에 참여함.", "~을 수행함.", "~을 발표함." (주제도, 관점도, 결과물의 특징도 드러나지 않음) → 이 경우에만 개선이 필요한 문장으로 지적하세요.
                      (2) 구체적인 활동 서술 자체로 역량이 이미 드러나는 문장: 활동의 주제·문제의식·대상·접근 방식이 충분히 구체적이어서, 그 서술 자체만으로도 학생의 관심사·문제의식·사고 수준이 드러나는 문장. 예: "이주배경 학생이 학교 현장에서 겪는 '행정적 타자화' 실태를 다룬 정책 제안 보고서를 작성함." → 이런 문장은 이미 주제 선정의 예리함과 사회적 문제의식이 문장 자체에서 드러나므로, 그대로 좋은 문장(2번 항목 우수 사례)으로 인정하세요. 여기에 "비판적으로 인식하고 창의적인 대안을 제시하려는 노력이 돋보임" 같은 상투적이고 일반적인 평가 문구를 억지로 덧붙이도록 수정 제안하지 마세요. 오히려 이런 식으로 뻔한 미사여구를 인위적으로 붙이면 5번(AI 어색함) 기준에 위배되는 부자연스러운 문장이 됩니다.
                    - 즉, "관찰·평가가 없다"는 이유만으로 잘 쓰인 문장을 개선 대상으로 삼지 마세요. 개선이 필요한 문장은 오직 (1)처럼 활동의 구체성 자체가 결여되어 학생의 특성을 전혀 알 수 없는 경우로 한정하세요.
                    - 수정 예시를 제시할 때도, 이미 구체적인 문장에 뻔한 평가어를 덧붙이는 방식이 아니라, (1)에 해당하는 진짜 밋밋한 문장에 대해서만 그 활동에서 드러난 구체적 특이점(접근 방식, 사고 과정, 결과물의 차별점 등)을 자연스럽게 보강하는 방식으로 제시하세요.
                    - good_points에서 2번 항목 우수 사례를 꼽을 때는, 교사의 명시적 평가어가 붙은 문장뿐 아니라 (2)처럼 구체적 서술 자체로 역량이 드러나는 문장도 함께 인용하며 왜 그 문장이 좋은지(주제의 구체성, 문제의식의 수준 등) 짚어주세요.
                    ...
                    2. 교사의 관찰이 들어간 기록인가 (20점 만점) — 활동의 주제·대상·방식이 구체적으로 드러나 학생의 특성이 짐작되는지가 핵심이며, 매 문장에 평가어가 붙어야 하는 것은 아님 (밋밋하고 막연한 행위 나열만 감점 대상)
                    ...
                    {{
                        "setuk_eval": {{
                            ...
                            "good_points": "1. (수학) '이차함수의 최댓값을 구하는 과정에서 판별식을 적용하여 풀이 과정을 논리적으로 서술함' — 개념 적용의 정확성이 돋보임\\n2. (통합과학) '실험 중 변인 통제의 필요성을 스스로 제기함' — 교사의 관찰과 평가가 함께 담긴 문장\\n3. (사회) '이주배경 학생이 학교 현장에서 겪는 행정적 어려움을 다룬 정책 제안 보고서를 작성함' — 주제 선정 자체에서 문제의식의 구체성과 사회적 감수성이 이미 드러나므로 별도의 평가어 없이도 좋은 문장",
                            "improvements": "▍[2번. 교사 관찰 결여]\\n다음 2개 문장은 활동의 주제·대상·방식이 전혀 구체적이지 않아 학생의 특성을 전혀 짐작할 수 없음(주의: 이미 주제가 구체적인 문장은 여기 포함하지 말 것):\\n1. (수학) '수업 내용에 대해 조사하여 발표함.'\\n2. (영어) '팀 프로젝트에 참여함.'\\n\\n▍[6-a. 수식어구 중첩]\\n다음 2개 문장에서 수식어가 과도하게 중첩되어 핵심 내용이 바로 읽히지 않음:\\n1. (음악) '창작 과정에서 랩을 작사하는 것에 어려움을 겪기도 했으나, 모둠원들과 다각도로 아이디어를 나누고 다양한 힙합 곡들을 탐색 및 분석하여 완성도 높은 수준의 곡을 완성해 냄.'\\n2. (음악) '팀원 간 의견 조율이 필요한 상황마다 특유의 리더십과 매끄러운 진행 능력으로 분위기를 유연하게 풀어나갔으며, 가사의 분위기와 조화를 이루는 최적의 비트를 찾음.'\\n\\n(실제 응답에서는 위와 같은 형식으로 [2번]~[6-d]까지 해당되는 모든 항목을, 발견되는 모든 문제 문장을 빠짐없이 담아 작성할 것. 절대 줄바꿈 없이 이어붙이지 말 것. 단, '-습니다'체로 고치라는 피드백은 절대 포함하지 말 것. 이미 주제·대상이 구체적으로 드러난 문장에 상투적 평가어를 억지로 덧붙이라는 식의 피드백은 절대 포함하지 말 것)",
                            "revision_examples": "▍[2번. 교사 관찰 결여]\\n(20103) 수정 전: '수업 내용에 대해 조사하여 발표함.'\\n(20103) 수정 후: '이차함수 그래프를 직접 그려보며 대칭성과 함수식의 관계를 스스로 도출해내는 논리적 사고력을 보임.' (막연한 '수업 내용'을 실제 다룬 구체적 주제로 대체하여 보강)\\n동일 유형이므로 같은 방식으로 수정 가능한 문장: (20108) '팀 프로젝트에 참여함.'\\n\\n▍[6-a. 수식어구 중첩]\\n(20111) 수정 전: '창작 과정에서 랩을 작사하는 것에 어려움을 겪기도 했으나, 모둠원들과 다각도로 아이디어를 나누고 다양한 힙합 곡들을 탐색 및 분석하여 완성도 높은 수준의 곡을 완성해 냄.'\\n(20111) 수정 후: '창작 과정에서 랩 작사에 어려움을 겪음. 이에 모둠원들과 다각도로 아이디어를 나누고 다양한 힙합 곡들을 탐색 및 분석하며 완성도 높은 곡을 완성하는 모습을 보임.'\\n\\n(improvements에서 정리한 모든 항목·모든 그룹에 대해 위와 같은 형식으로, 모든 수정 전/후 문장 앞에 학번을 빠짐없이 표기하며 이어서 작성할 것)"
                        }}
                    }}
                    """
                ...
                # [분기 1-2] 동아리 특기사항 전용 피드백
                    ...
                    [2번 항목(교사의 관찰) 판별 기준 - 세특 전체 관점에서 균형 있게 적용]:
                    - 2번 항목은 "문장 하나하나마다" 평가어가 붙어야 한다는 뜻이 아닙니다. 반드시 매 문장을 "~보임", "~돋보임" 식으로 뜯어고칠 필요는 없습니다.
                    - 다음을 구분하세요:
                      (1) 진짜로 밋밋한 행위 나열: 활동의 주제·대상·방식이 전혀 구체적이지 않아 학생의 특성을 전혀 짐작할 수 없는 문장. 예: "동아리 활동에 참여함.", "역할을 수행함.", "내용을 발표함." → 이 경우만 개선 대상으로 지적하세요.
                      (2) 구체적인 활동 서술 자체로 역량이 이미 드러나는 문장: 활동의 주제·문제의식·접근 방식이 충분히 구체적이어서 그 서술만으로도 학생의 관심사와 사고 수준이 드러나는 문장 → 이런 문장은 상투적 평가어를 억지로 덧붙이지 말고 그대로 좋은 문장(우수 사례)으로 인정하세요. 뻔한 미사여구를 인위적으로 붙이면 5번(AI 어색함) 기준에 위배됩니다.
                    - good_points에서 2번 항목 우수 사례를 꼽을 때는 교사의 명시적 평가어가 붙은 문장뿐 아니라 (2)처럼 구체적 서술 자체로 역량이 드러나는 문장도 함께 인용하세요. 개선이 필요한 문장은 오직 (1)에 해당하는, 활동의 구체성 자체가 결여된 경우로 한정하세요.
                    ...
                    - [매우 중요] revision_examples의 모든 "수정 전"/"수정 후" 문장 앞에도 improvements에서와 동일하게 그 문장이 속한 학번(또는 이름 등 원문의 식별자)을 반드시 표기하세요. 예: "(20103) 수정 전: '...'" / "(20103) 수정 후: '...'" 처럼 두 줄 모두에 동일한 식별자를 붙여 어느 학생의 문장을 어떻게 고쳤는지 한눈에 대응되도록 하세요.
                    ...
                    2. 교사의 관찰이 들어간 기록인가 (15점 만점) — 활동의 주제·대상·방식이 구체적으로 드러나 학생의 특성이 짐작되는지가 핵심이며, 매 문장에 평가어가 붙어야 하는 것은 아님 (밋밋하고 막연한 행위 나열만 감점 대상)
                    ...
                    {{
                        "club_eval": {{
                            ...
                            "good_points": "1. (김OO) '토론 진행 중 상반된 의견을 정리하여 절충안을 제시함' — 리더십과 조율 능력에 대한 교사의 평가가 담긴 문장\\n2. (이OO) '데이터 분석 결과의 한계를 스스로 지적함' — 비판적 사고에 대한 관찰이 드러남\\n3. (한OO) '지역 내 다문화가정 자녀의 방과후 돌봄 공백 문제를 주제로 개선 방안을 제안함' — 주제 선정 자체에서 문제의식의 구체성이 이미 드러나므로 별도 평가어 없이도 좋은 문장",
                            "improvements": "▍[2번. 관찰 결여]\\n다음 문장은 활동의 주제·대상·방식이 전혀 구체적이지 않아 학생의 특성을 짐작할 수 없음(주의: 이미 주제가 구체적인 문장은 여기 포함하지 말 것):\\n1. (박OO) '동아리 활동에 참여함.'\\n2. (최OO) '자료를 조사하여 보고서를 작성함.'\\n\\n▍[6번. 가독성]\\n다음 문장에서 수식어가 과도하게 중첩되어 가독성이 저하됨:\\n1. (정OO) '팀원 간 의견 조율이 필요한 상황마다 특유의 리더십과 매끄러운 진행 능력으로 분위기를 유연하게 풀어나갔으며, 가사의 분위기와 조화를 이루는 최적의 비트를 찾음.'\\n\\n(실제 응답에서는 위와 같은 형식으로 해당되는 모든 항목에 대해, 발견되는 모든 문제 문장을 빠짐없이 담아 작성할 것. 단, '-습니다'체로 고치라는 피드백은 절대 포함하지 말 것. 이미 주제·대상이 구체적으로 드러난 문장에 상투적 평가어를 억지로 덧붙이라는 식의 피드백은 절대 포함하지 말 것)",
                            "revision_examples": "▍[2번. 관찰 결여]\\n(김OO) 수정 전: '동아리 활동에 참여함.'\\n(김OO) 수정 후: '지역 아동센터 방문 봉사 계획 수립 과정에서 예산 배분을 두고 의견이 갈리자 절충안을 제시하는 조율 능력을 보임.' (막연한 '활동'을 실제 다룬 구체적 상황으로 대체하여 보강)\\n동일 유형이므로 같은 방식으로 수정 가능한 문장: (이OO) '자료를 조사하여 보고서를 작성함.'\\n\\n(improvements에서 정리한 모든 항목·모든 그룹에 대해 위와 같은 형식으로, 모든 수정 전/후 문장 앞에 학번(또는 이름)을 빠짐없이 표기하며 이어서 작성할 것)"
                        }}
                    }}
                    """
                ...
                # [분기 2] 교사 전용 (생기부 종합) 피드백 (교사 기록 적절성, 가독성, 성적 수준 적절성, AI 남용 여부 및 향후 지도 방향 포함)
                    ...
                    3. 학생 성적과 활동에 대한 교사의 기록이 적절했는지 상세히 진단하세요:
                       - 교사의 관찰(구체적 행동이나 수업 태도 등)이 잘 드러났는지. 단, 이는 문장마다 평가어가 붙어야 한다는 뜻이 아닙니다. "~에 참여함", "~을 수행함", "~을 발표함"처럼 주제·대상·방식이 전혀 구체적이지 않아 학생의 특성을 전혀 짐작할 수 없는 밋밋한 문장만 감점 대상으로 삼으세요. 이미 활동의 주제나 문제의식이 구체적으로 드러나는 문장은 그 자체로 좋은 기록으로 인정하고, 상투적 평가어를 억지로 덧붙이라는 피드백은 하지 마세요.
                ...
                try:
                    ...
                    for _outer_key in ("setuk_eval", "club_eval"):
                        if _outer_key in result_json and isinstance(result_json[_outer_key], dict):
                            ...
                            # improvements에 표기된 "(학번) '문장'" 매핑을 근거로,
                            # revision_examples에서 학번 표기가 누락된 동일 문장에 자동으로 학번을 보강
                            if "improvements" in result_json[_outer_key] and "revision_examples" in result_json[_outer_key]:
                                result_json[_outer_key]["revision_examples"] = inject_student_ids(
                                    result_json[_outer_key]["improvements"],
                                    result_json[_outer_key]["revision_examples"]
                                )
